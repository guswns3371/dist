...
$m_desc = $_POST['m_desc'];
$holder_img = $_POST['holder'];
if(strpos($m_desc,"'") !== false){
    $m_desc = str_replace("'","",$m_desc); //문자열에 ' 문자가 있으면 mysql 신택스 문제가 생긴다
}

error_log("music_edit : ".$filename."/".$uploader."/".$m_title."/".$m_genre."/".$m_artist."/".$m_desc."!");

$folder = "/var/www/html/music_upload/".$trim_filename;
...
//    $res_2["picture"]=$file_2['id3v2']['APIC'][0]['data'];// 바이너리 이미지 코드
    ...
    if($file['id3v2'] == ""){ // 파일정보를 빼낼수 없을 떄
        ...
        if(($m_title == "") ||($m_genre == "") ||($m_artist == "")){
            ...
            m_artist = '{$m_artist}',
            m_descript = '{$m_desc}',
            m_image = '{$res["real_picture"]}',
            m_date =  '{$date}'
            WHERE m_filename='$filename' AND m_uploader='$uploader' 
            ";
                $result2 = mysqli_query($conn,$sql2);
                error_log("music edit error2 :".mysqli_error($conn));
                if($result2 === false){//제대로 수정이 안 되었을떄
                    $alert ="1";
                }else{ // 제대로 수정이 되었을떄
                    $alert = "2";
                }
...
<?php if($alert == "0"){
    ?>
    <script type="text/javascript">alert("내용을 모두 입력해 주세요!2");</script>
...
<?php } else if(($alert == "1")){?>
    <script type="text/javascript">alert("저장하는 과정에서 문제가 생겼습니다. 관리자에게 문의 하세요.2");</script>
    <meta http-equiv="refresh" content="0 url=music_upload.php?filename=<?php echo urlencode($filename)?>&title=<?php echo urlencode($res["title"])?>&artist=<?php echo urlencode($res["artist"])?>&genre=<?php echo urlencode($res["genre"])?>&image_mime=<?php echo $img_mime?>"/>
<?php } else if($alert == "2"){?>
    <script type="text/javascript">alert("수정이 완료되었습니다.");</script>
    <meta http-equiv="refresh" content="0 url=music_innerpage.php?m_idx=<?php echo $m_idx?>"/>
<?php } else if($alert == "3"){?>
    <script type="text/javascript">alert("파일이 너무 큽니다2");</script>
    <meta http-equiv="refresh" content="0 url=music_upload.php?filename=<?php echo urlencode($filename)?>&title=<?php echo urlencode($res["title"])?>&artist=<?php echo urlencode($res["artist"])?>&genre=<?php echo urlencode($res["genre"])?>&image_mime=<?php echo $img_mime?>"/>
<?php } else if($alert == "4"){?>
    <script type="text/javascript">alert("파일이 첨부되지 않았습니다2");</script>
    <meta http-equiv="refresh" content="0 url=music_upload.php?filename=<?php echo urlencode($filename)?>&title=<?php echo urlencode($res["title"])?>&artist=<?php echo urlencode($res["artist"])?>&genre=<?php echo urlencode($res["genre"])?>&image_mime=<?php echo $img_mime?>"/>
<?php } else if($alert == "5"){?>
    <script type="text/javascript">alert("파일이 제대로 업로드되지 않았습니다2");</script>
    <meta http-equiv="refresh" content="0 url=music_upload.php?filename=<?php echo urlencode($filename)?>&title=<?php echo urlencode($res["title"])?>&artist=<?php echo urlencode($res["artist"])?>&genre=<?php echo urlencode($res["genre"])?>&image_mime=<?php echo $img_mime?>"/>
<?php } else if($alert == "6"){?>
    <script type="text/javascript">alert("지원하지 않는 파일 형식입니다.2");</script>
    <meta http-equiv="refresh" content="0 url=music_upload.php?filename=<?php echo urlencode($filename)?>&title=<?php echo urlencode($res["title"])?>&artist=<?php echo urlencode($res["artist"])?>&genre=<?php echo urlencode($res["genre"])?>&image_mime=<?php echo $img_mime?>"/>
<?php }?>
