...
if(isset($_POST['getData'])){
    ...
    $m_uploader = $conn->real_escape_string($_POST['m_uploader']);


    $sql00 = $conn->query("select * from MusicInfo where m_uploader = '$m_uploader'order by m_idx desc LIMIT $start,$limit");
    error_log("mymusic infinite mun row _ m_uploader : ".$m_uploader."/".$start."/".$limit);
    if($sql00->num_rows >0){
        $response = "";

        while ($board = $sql00->fetch_array()){
            ////////////////////////////////////
 ...
 <div class="" style="margin-bottom: 20px">
  ...
  <div class="media-body-2" id="media-body" style="margin-left: 30px">
        ...
        error_log("mymusic infinite num row : NO_reachedMax /".$sql00->num_rows);
        exit($response);
    }else{
        error_log("mymusic infinite num row : YES_reachedMax /".$sql00->num_rows);
        exit('reachedMax');
    }
}
?>