...
$sql =mq("select * from FollowUser where following_user_id= '$followed_user_id'");
...
<div class="" id="background_img" style="width: 1700px;height: 400px;margin:0 auto;">
    <div class="media-body" >
        ...
        <label>
            <h1 class="d-inline-flex p-2" style="background-color: #505759;color: white; margin-top: 50px;margin-left: 30px">❤ <?php echo $rep_count_9?> people who</h1>
            <div></div>
            <h1 class="d-inline-flex p-2" style="background-color: #505759;color: white; margin-top: 0px;margin-left: 30px"><?php echo $followed_user_id?> is following</h1>
                ...
                <ul>
                    <li>
                        ...
                        while ($row = $sql->fetch_array()){
                            $followed_user_id__ = $row['followed_user_id']; //팔로우 당한사람
                            $following_user_id__ = $row['following_user_id'];//팔로우 하는 사람

                            $sql9_ = mq("select * from UserInfo where id ='$followed_user_id__' order by Userkey desc");
