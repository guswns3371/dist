...
if(isset($_POST['getData'])){
    ...
    $search_m = $conn->real_escape_string($_POST['search_m']);
    $sql00 = $conn->query("SELECT * FROM MusicInfo WHERE m_title LIKE '%$search_m%' OR m_artist LIKE '%$search_m%' OR m_uploader LIKE '%$search_m%' OR m_genre LIKE '%$search_m%' ORDER BY m_idx desc LIMIT $start,$limit");
    if($sql00->num_rows >0){
        $response = "";
